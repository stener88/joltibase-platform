 ...
 * 
 * Includes image processing, HTML escaping, and placeholder generation
 */
 ...
 */
export function processImageUrl(url: string, type: 'logo' | 'image' | 'hero' = 'image'): string {
  // Check if URL is a merge tag placeholder
  if (/^\{\{.+\}\}$/.test(url)) {
    return getPlaceholderImage(
      EMAIL_DIMENSIONS.MAX_WIDTH,
      PLACEHOLDER_DIMENSIONS.IMAGE.HEIGHT,
      type === 'hero' ? 'image' : type
    );
  }
...
export function getPlaceholderAvatar(): string {
  const size = PLACEHOLDER_DIMENSIONS.AVATAR.SIZE;
  return `data:image/svg+xml,${encodeURIComponent(`
    <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}">
      <circle cx="${size / 2}" cy="${size / 2}" r="${size / 2}" fill="${PLACEHOLDER_COLORS.IMAGE_BG}"/>
      <circle cx="${size / 2}" cy="${size / 2 - 4}" r="10" fill="${PLACEHOLDER_COLORS.ICON}"/>
      <path d="M${size / 4} ${size - 14} Q${size / 2} ${size - 24} ${(size * 3) / 4} ${size - 14}" stroke="${PLACEHOLDER_COLORS.ICON}" stroke-width="2" fill="none"/>
    </svg>
  `)}`;
}
 ...
 */
export function processAvatarUrl(url: string | null | undefined): string {
  if (!url) {
 ...
 */
export function getOutlookArcsize(borderRadius: string): string {
