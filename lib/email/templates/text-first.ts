 ...
 * Text First Template
 * 
 * Pure content focus, minimal styling, maximum readability
 * Best for: Newsletters, content-heavy emails, personal messages
 */

import type { TemplateRenderInput } from './types';
import { getFontStack, replaceMergeTags, getTypography, getSpacing } from './types';

export function renderTextFirst(input: TemplateRenderInput): string {
  ...
  // Design system - Text First uses minimal scale for understated elegance
      ...
      switch (section.type) {
        ...
        case 'stats':
          const statsHtml = (section.stats || [])
            .map(stat => `
              <div style="display: inline-block; margin: 0 24px ${spacing.elementSpacing} 0; text-align: center;">
  ...
  // Build footer social links if provided
  const socialLinksHtml = content.footer?.socialLinks
    ?.map(link => `<a href="${link.url}" style="color: #6b7280; text-decoration: none; margin-right: 16px;">${link.platform}</a>`)
    .join('') || '';
...
<body style="margin: 0; padding: 0; font-family: ${fontStack}; background-color: #ffffff; line-height: 1.6;">
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="max-width: 600px; width: 100%;">
          ...
          <tr>
            <td style="padding: ${spacing.outerPadding} 40px;">
              ...
              <!-- CTA -->
              <div style="margin: ${spacing.sectionSpacing} 0;">
                <a href="${content.cta.url}" style="color: ${ctaColor}; text-decoration: underline; font-size: ${typography.body}; font-weight: 600;">
                  ${content.cta.text} →
                </a>
              </div>
          ...
          <tr>
            <td style="padding: 24px 40px; border-top: 1px solid #e5e7eb;">
              ...
              <p style="margin: 0; font-size: 13px; color: #9ca3af;">
                ...
                ${content.footer?.companyName ? ` · <a href="{{preferences_url}}" style="color: #6b7280; text-decoration: underline;">Preferences</a>` : ''}
              </p>
