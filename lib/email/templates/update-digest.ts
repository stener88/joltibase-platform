 ...
 * 
 * Organized multi-section layout for news and updates
 * Best for: Company updates, product changelogs, weekly/monthly digests, news roundups
 * 
 * Design Philosophy:
 * - Highly organized sections
 * - Easy to scan structure
 * - Numbered/categorized updates
...
export function renderUpdateDigest(input: TemplateRenderInput): string {
  ...
  const backgroundColor = design.backgroundColor || '#f6f7f9';
  
  // Update Digest uses standard scale with compact spacing for content density
  const typography = getTypography(design.typographyScale || 'standard');
  const spacing = getSpacing(design.layoutVariation?.spacing || 'compact');
    ...
    .map((section) => {
      switch (section.type) {
        case 'heading':
          sectionCounter++;
          return `
            <div style="margin: ${spacing.sectionSpacing} 0 20px; padding: 16px 20px; background: linear-gradient(135deg, ${ctaColor}12, ${accentColor}12); border-left: 4px solid ${ctaColor}; border-radius: 6px;">
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="width: 40px; vertical-align: middle;">
                    <div style="width: 36px; height: 36px; background-color: ${ctaColor}; color: #ffffff; border-radius: 50%; text-align: center; line-height: 36px; font-weight: 800; font-size: 16px;">
                      ${sectionCounter}
                    </div>
                  </td>
        ...
        case 'divider':
          return `<hr style="margin: ${spacing.sectionSpacing} 0; border: none; border-top: 2px dashed #e2e8f0;" />`;
        ...
        case 'hero':
          return `
            <div style="margin: 0 0 ${spacing.sectionSpacing}; padding: ${spacing.elementSpacing}; background-color: #ffffff; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); text-align: center;">
              <h2 style="margin: 0 0 12px; font-size: ${typography.h2}; font-weight: ${typography.weight.headline}; color: #1a202c; line-height: 1.2;">${section.headline || ''}</h2>
              ${section.subheadline ? `<p style="margin: 0; font-size: 18px; color: #718096; line-height: 1.6;">${section.subheadline}</p>` : ''}
            </div>
          `;
        ...
        case 'feature-grid':
          const featuresHtml = (section.features || [])
            .map(feature => `
              <div style="margin-bottom: 20px; padding: 20px; background-color: #ffffff; border-left: 3px solid ${accentColor}; border-radius: 6px;">
        ...
        case 'testimonial':
          ...
          return `
            <div style="margin: ${spacing.sectionSpacing} 0; padding: 24px; background-color: #ffffff; border-left: 4px solid ${ctaColor}; border-radius: 8px;">
              <p style="margin: 0 0 16px; font-size: 18px; color: #2d3748; line-height: 1.7; font-style: italic;">
        ...
        case 'stats':
          const statsHtml = (section.stats || [])
            .map(stat => `
              <div style="display: inline-block; margin: 0 20px 16px 0; text-align: center; padding: 20px; background-color: #ffffff; border-radius: 10px; min-width: 120px;">
        ...
        case 'comparison':
          ...
          return `
            <table cellpadding="0" cellspacing="0" style="width: 100%; margin: 24px 0; border-collapse: collapse;">
              <tr>
                <td style="width: 50%; padding: 20px; background-color: #ffffff; border-radius: 8px; vertical-align: top;">
                  <p style="margin: 0 0 10px; font-size: 12px; font-weight: 700; color: #a0aec0; text-transform: uppercase;">Previous</p>
                  <p style="margin: 0; font-size: ${typography.body}; color: #4a5568; line-height: 1.6;">${section.comparison.before}</p>
                </td>
                <td style="width: 16px;"></td>
                <td style="width: 50%; padding: 20px; background-color: #ffffff; border: 2px solid ${ctaColor}; border-radius: 8px; vertical-align: top;">
                  <p style="margin: 0 0 10px; font-size: 12px; font-weight: 700; color: ${ctaColor}; text-transform: uppercase;">Updated</p>
        ...
        case 'cta-block':
          return `
            <div style="margin: ${spacing.sectionSpacing} 0; padding: 24px; background-color: #ffffff; border-radius: 10px; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.06);">
              ${section.content ? `<p style="margin: 0 0 20px; font-size: ${typography.body}; color: #4a5568; line-height: 1.6;">${section.content}</p>` : ''}
              <a href="${section.ctaUrl || '{{cta_url}}'}" style="display: inline-block; padding: 14px 32px; font-size: ${typography.body}; font-weight: 700; color: #ffffff; background-color: ${ctaColor}; text-decoration: none; border-radius: 6px;">
          ...
          <tr>
            <td style="padding: 0 0 24px; text-align: center;">
              <table cellpadding="0" cellspacing="0" style="margin: 0 auto;">
                <tr>
                  <td style="padding: 10px 24px; background-color: ${ctaColor}; border-radius: 24px;">
                    <p style="margin: 0; font-size: 12px; font-weight: 800; color: #ffffff; text-transform: uppercase; letter-spacing: 1.5px;">
                      📰 UPDATE DIGEST
                    </p>
                  </td>
                </tr>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="padding: ${spacing.outerPadding} 40px;">
                    ...
                    <!-- Headline -->
                    <h1 style="margin: 0 0 16px; font-size: ${typography.h1}; font-weight: ${typography.weight.headline}; color: #1a202c; line-height: 1.2; letter-spacing: -0.02em;">
                      ${content.headline}
                    </h1>
                    ...
                    <!-- Divider -->
                    <hr style="margin: 0 0 ${spacing.sectionSpacing}; border: none; border-top: 2px solid #e2e8f0;" />
                    
                    <!-- Body Sections -->
                    ${sectionsHtml}
                    
                    <!-- Main CTA -->
                    <div style="margin: ${spacing.sectionSpacing} 0 0; text-align: center; padding-top: ${spacing.elementSpacing}; border-top: 2px dashed #e2e8f0;">
